    ...
    public class Game_Grid
    {
        ...
        public Check_Line()
        {
            ...
            for (int i = 17; i <= 0; i--)
            {
                ...
                for (int j = 0; j < 10; j++)
                {
                    if (field[i][j]) //Checks if there is a peice of the object on that spot
                    {
                        count++;
                        if(count == 10) //if there is a peice of the object for that whole row, clear it
                        {
                            for (int j = 0; j < 10; j++)
                            {
                                field[i][j] = field[i - 1][j]; //deletes by copying the top row
                            }
                            clear = true;
                            i++;
                        }
        ...
        public Falling_Block(int[,] block, int row, int column)
        {
            bool falling = true;
            while (falling)
            {
                ...
                for (int i = 0; i < 18; i++)
                {
                    ...
                    if (column == 4)//straight line only 
                    {
                        ...
                        if(field[i][middle -1] && field[i][middle] && field[i][middle +1] && field[i][middle + 2])
                        {
                            falling = false;
                            break;
                        }
